...
def preprocess_input(user_input):
    # Convert to lowercase and remove special characters except for spaces
    user_input = user_input.lower()
    user_input = re.sub(r'[^\w\s\'"]', '', user_input)  # Remove special characters except for alphanumeric, spaces, single quotes, and double quotes
    return user_input

def chatbot_response(user_input):
    user_input = preprocess_input(user_input)

    # Check if the chatbot's name is mentioned
    if re.search(r'\bshowbaby\b', user_input):
        user_input = re.sub(r'\bshowbaby\b', '', user_input).strip()
        if not user_input:
            return "You called me, but I didn't hear anything. How can I assist you?"
...
# Chat loop
print("Hello! I'm Showbaby. How can I help you today?")
